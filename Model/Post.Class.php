...
class Model_Post
{
	...
	{
		return $this->db->query("select post.id, post.title, SUBSTR(post.content,1,500) as content, post.date, post.date_update, post.author_id, post.cat_id, post.image, user.name from post inner join user on post.author_id=user.id ORDER BY post.date DESC LIMIT ".$offset.", ".$limit);
	}
